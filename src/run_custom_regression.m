function run_custom_regression(func, func_str, options=[])
  ...
  legend('True Function', 'Neural Network Prediction');
  ...
  if n_segments > 1
    for i = 1:n_segments
      range_i = sub_ranges{i};
      yrange = get(gca, 'YLim');
      line([range_i(1), range_i(1)], yrange, 'LineStyle', '--', 'Color', [0.5, 0.5, 0.5]);
      line([range_i(2), range_i(2)], yrange, 'LineStyle', '--', 'Color', [0.5, 0.5, 0.5]);
    endfor
  endif
